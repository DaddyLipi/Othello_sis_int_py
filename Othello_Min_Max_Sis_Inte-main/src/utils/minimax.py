...
def max_value_ab(state, alpha, beta): 
    ...
    for a in temp_board.all_legal_moves(1):   
        sucessor = result(state, a, 'max') 
        eval_min = min_value_ab(sucessor, alpha, beta)
        v=max(v,eval_min)
        alpha=max(alpha,v)
        if(alpha >= beta): break
...
def min_value_ab(state, alpha, beta):
    ...
    for a in temp_board.all_legal_moves(-1):  
        sucessor = result(state, a, 'min') 
        eval_max = max_value_ab(sucessor, alpha, beta) 
        v = min(v, eval_max)
        beta = min(beta, v)
        if(alpha >= beta): break 
...
def min_max_ab(state, player):
    if(player == "max"): 
        ...
        for a in actions: 
            sucessor = result(state.board, a, player)
            val = min_value_ab(sucessor, -1000, 1000)
            v.append(val)
        v = np.array(v)
        index = np.argmax(v)
        return actions[index]
        ...
        for a in actions.copy(): 
            sucessor = result(state, a, player)
            val = max_value_ab(sucessor, -1000, 1000)
            v.append(val)
        v = np.array(v)
        index = np.argmin(v)
        return actions[index]
